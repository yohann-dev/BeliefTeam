...
export default function Projects() {
  ...
  return (
    <div className="bg-white from-white to-meme-blue-muted py-12 px-4 sm:px-6 lg:px-8 relative min-h-screen flex flex-col">
      <BackButton />
      ...
      <div className="max-w-7xl mx-auto w-full flex-1 flex flex-col">
        <div className="mb-8">
          <h1 className="text-4xl font-extrabold text-center">
            ...
            <span className="block bg-gradient-to-r from-meme-blue to-meme-blue-accent bg-clip-text text-transparent animate-pulse-slow">Believe Projects</span>
          </h1>
        </div>
            ...
            <div className="mb-6 flex flex-col sm:flex-row gap-4 justify-between items-center">
              <div className="w-full sm:w-96">
                <input
                  ...
                  className="w-full px-4 py-2 rounded-xl border-2 border-meme-blue-muted bg-white text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-meme-blue-accent focus:border-transparent"
                />
              </div>
              <button
                ...
                className={`px-4 py-2 rounded-xl font-medium transition-all duration-300 ${
                  showOnlyWithDetails
                    ? 'bg-gradient-to-r from-meme-blue to-meme-blue-accent text-white shadow-meme-glow'
                    : 'bg-white text-meme-blue border-2 border-meme-blue-muted hover:from-meme-blue hover:to-meme-blue-accent hover:bg-gradient-to-r hover:text-white'
                }`}
              >
            ...
            <div className="flex-1 min-h-[500px] flex flex-col">
                ...
                <table className="min-w-full divide-y divide-gray-200 rounded-2xl overflow-hidden shadow-meme bg-white">
                  <thead className="bg-gradient-to-r from-meme-blue to-meme-blue-accent sticky top-0 z-10">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-left text-xs font-semibold text-white uppercase tracking-wider w-[80px] min-w-[80px] max-w-[80px]">Contract Address</th>
                      <th
                    ...
                    {paginatedTokens.map((token, idx) => (
                      <tr key={token.tokenAddress} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-meme-blue-muted'} hover:bg-meme-blue-muted/50 transition-colors`}>
                        <td className="px-4 py-3 whitespace-nowrap font-semibold text-meme-blue w-[150px] min-w-[150px]">
                          <a href={`https://believe.app/coin/${token.tokenAddress}`} target="_blank" rel="noopener noreferrer" className="hover:text-meme-blue-accent transition-colors">
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-meme-blue w-[120px] min-w-[120px] max-w-[120px] truncate">
                          <a href={`https://x.com/${token.author}`} target="_blank" rel="noopener noreferrer" className="hover:text-meme-blue-accent transition-colors">
                            @{token.author}
                          </a>
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-gray-500 w-[120px] min-w-[120px] max-w-[120px]">
                          <div className="flex items-center gap-2">
                            <span className="truncate">{token.tokenAddress}</span>
                            <button
                              onClick={() => navigator.clipboard.writeText(token.tokenAddress)}
                              className="flex-shrink-0 inline-flex items-center justify-center w-8 h-8 rounded-full bg-gradient-to-r from-meme-blue to-meme-blue-accent text-white hover:from-meme-blue-accent hover:to-meme-blue transition-colors shadow-sm"
                        ...
                        <td className="px-4 py-3 whitespace-nowrap text-center text-meme-blue-dark w-[120px] min-w-[90px]">
                          {token.marketData?.holderCount ? token.marketData.holderCount.toLocaleString() : '-'}
                        </td>
                        <td className="px-4 py-3 whitespace-nowrap text-center w-[120px] min-w-[90px]">
                          <div className="flex justify-center gap-2">
                            <button
                              onClick={() => setSelectedToken(token)}
                              className="inline-flex items-center px-3 py-1.5 text-sm font-medium rounded-lg text-white bg-gradient-to-r from-meme-blue to-meme-blue-accent hover:from-meme-blue-accent hover:to-meme-blue focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-meme-blue-accent transition-all duration-200"
                            >
                              Details
                            </button>
                            <button
                              onClick={() => {
                                window.location.href = `/projects/${token.tokenAddress}/boost`;
                              }}
                              className="inline-flex items-center px-3 py-1.5 border border-transparent text-sm font-medium rounded-lg text-white bg-gradient-to-r from-orange-500 via-red-500 to-orange-500 hover:from-orange-600 hover:via-red-600 hover:to-orange-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-orange-500 transition-all duration-200"
                            >
                              Boost
                            </button>
                          </div>
              ...
              <div className="flex justify-center items-center gap-4 mt-6 py-4">
                <button
                  ...
                  className="p-2 rounded-xl bg-white text-meme-blue border-2 border-meme-blue font-semibold hover:bg-gradient-to-r hover:from-meme-blue hover:to-meme-blue-accent hover:text-white transition-colors disabled:opacity-50"
          ...
          <div className="bg-white rounded-2xl p-6 max-w-2xl w-full max-h-[90vh] overflow-y-auto shadow-meme-glow">
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <p className="bg-gradient-to-r from-meme-blue to-meme-blue-accent bg-clip-text text-transparent font-medium">${selectedToken.tokenSymbol}</p>
              </div>
              <button
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-sm font-medium text-meme-blue-accent">Contract Address</h3>
                <p className="mt-1 text-sm text-gray-900">{selectedToken.tokenAddress}</p>
              </div>

              {selectedToken.description && (
                <div>
                  <h3 className="text-sm font-medium text-meme-blue-accent">Description</h3>
              ...
              {selectedToken.needs && selectedToken.needs.length > 0 && (
                <div>
                  <h3 className="text-sm font-medium text-meme-blue-accent">Project Needs</h3>
                  <div className="mt-2 flex flex-wrap gap-2">
                    {selectedToken.needs.map((need, index) => (
                      <span key={index} className="px-3 py-1 text-sm font-medium text-meme-blue-accent bg-meme-blue-muted rounded-full border border-meme-blue-accent">
                        {need}
                      </span>
                    ))}
                  </div>
              ...
              {selectedToken.tweetLink && (
                <div>
                  <h3 className="text-sm font-medium text-meme-blue-accent">Project link</h3>
                  <a
                    href={selectedToken.tweetLink}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-1 text-sm text-meme-blue hover:text-meme-blue-accent transition-colors"
                  >
                    {selectedToken.tweetLink}
                  </a>
                </div>
              ...
              {selectedToken.contactEmail && (
                <div>
                  <h3 className="text-sm font-medium text-meme-blue-accent">Contact Email</h3>
                  <a
              ...
              {selectedToken.extraInfo && (
                <div>
                  <h3 className="text-sm font-medium text-meme-blue-accent">Additional Information</h3>
